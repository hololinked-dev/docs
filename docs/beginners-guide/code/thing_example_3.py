...
class Picoscope6000(Picoscope):
    ...
    max_adc = Integer(
        doc="max value of the ADC possible for given resolution of data, 32512 for 8 bits & 32767 for higher bits.",
        readonly=True,
        default=32512,
    )  # type: int
    ...
    def get_unit_info(self, info_code: int) -> str:
        ...
        self._status["unit-info"] = ps.ps6000GetUnitInfo(
            self._ct_handle, string, length, req_size, info
        )
    ...
    def set_channel(
        ...
        bw_limiter = ps.PS6000_BANDWIDTH_LIMITER[
            "PS6000_BW_{}".format(bw_limiter.upper())
        ]
        self._status["set-channel-{}".format(channel)] = ps.ps6000SetChannel(
            self._ct_handle, ch, enabled, coupling, v_range, offset, bw_limiter
        )
    ...
    def get_timebase(
        ...
        self._status["get-timebase"] = ps.ps6000GetTimebase2(
            self._ct_handle,
            tb,
            no_samples,
            ctypes.byref(dtns),
    ...
    def run_block(
        ...
        self._status["run-block"] = ps.ps6000RunBlock(
            self._ct_handle,
            pre_trigger_samples,
    ...
    def check_ready(self):
        ...
        self._status["is-ready"] = ps.ps6000IsReady(
            self._ct_handle, ctypes.byref(ready)
        )
    ...
    def get_values(
        ...
        self._status["get_values"] = ps.ps6000GetValues(
            self._ct_handle,
            start_index,
            ctypes.byref(cnum_samples),
            downsample_ratio,
            downsample_mode,
            seg_index,
...
if __name__ == "__main__":
    ...
    try:
        ...
        props = [
            {"thresholdUpper": int(-3.8 / 5 * adcmax), "channel": "A"},
